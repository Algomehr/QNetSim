...
export const generateCircuitFromPrompt = async (prompt: string, forCode: boolean = false) => {
  ...
  if (forCode) {
      ...
      If the user asks for a circuit that utilizes advanced photonic components like 'waveplate', 'polarizer', 'pockelsCell', or 'eom', include those nodes and their corresponding parameters as defined in the schema.
      If the user specifies a custom gate or operation, create a 'custom' node and populate 'numQubits', 'customGateMatrix' (a JSON string of its unitary matrix), 'customNoiseModel', and 'customDescription'.
      `;
  } else {
      ...
      - 'custom' is for a user-defined quantum gate or operation. If you use it, include 'numQubits', 'customGateMatrix' (a JSON string of its unitary matrix, e.g., '[[[1,0],[0,0]],[[0,0],[1,0]]]'), 'customNoiseModel', and 'customDescription'.
      ...
      - For 'Waveplate', 'Polarizer', 'PockelsCell', 'EOM' components, set their specific parameters (e.g., 'retardance', 'fastAxisAngle', 'extinctionRatio', 'transmissionAxisAngle', 'voltage', 'halfWaveVoltage', 'modulationIndex', 'modulationFrequency').
...
If the circuit includes components like 'waveplate', 'polarizer', 'pockelsCell', or 'eom', describe their classical control or equivalent quantum operations in Qiskit if possible, or add comments indicating their function if direct Qiskit gates are not available. Prioritize implementing standard quantum gates.
If the circuit includes a 'custom' node, represent it as a custom unitary gate (using the 'customGateMatrix' if provided) in Qiskit, acting on 'numQubits'. Add comments about its 'customNoiseModel' and 'customDescription'.
`;
    ...
    properties: {
        ...
        errorSourceAnalysis: { // Structured output for error budget analysis
            ...
            items: {
                type: Type.OBJECT,
                properties: {
                    source: { type: Type.STRING, description: "The specific error source (e.g., 'Channel 1 Attenuation', 'Repeater 1 Memory T1', 'Detector 2 Crosstalk')." },
  ...
  switch (mode) {
    ...
    case 'performance':
      ...
      *** Analysis Focus: NETWORK PERFORMANCE ***
      ...
      - The 'protocolTrace' should log key events, especially probabilistic ones like photon loss in channels (considering all noise and free-space effects, as well as classical channel latency and bandwidth constraints), entanglement swap failures/successes at repeaters (considering swap fidelity, memory decoherence including T1/T2, amplitude/phase damping, storage time, storage efficiency, memory noise type), and state degradation due to various noise sources.
      - The 'resultsAnalysis' and 'errorSourceAnalysis' must focus on how physical parameters (channel length, attenuation, dispersion, PDL, thermal noise, depolarizing/dephasing rates, free-space turbulence, fading severity, repeater fidelity, memory decoherence, storage time, storage efficiency, memory noise type, detector efficiency, dark counts, dead time, afterpulsing, crosstalk, source photon statistics, indistinguishability, spectral purity, repetition rate, photonic component parameters, AND classical channel bandwidth/latency/error rate) directly impact the performance metrics. Provide a bottleneck analysis.`;
    case 'state':
      return `
      *** Analysis Focus: QUANTUM STATE ***
      ...
      - Analysis should focus on the quantum mechanical properties of the final state (e.g., degree of entanglement, purity, and how amplitude/phase damping, memory noise, and various channel/component imperfections affect them).
...
export const analyzeCircuitWithGemini = async (circuitJson: string, settings: SimulationSettings, mode: AnalysisMode) => {
    ...
    *** Core Simulation Instructions ***
    ...
    5.  **Detector Nodes:** Beyond 'efficiency', 'darkCounts', 'detectorType', 'interferometerArmLengthDifference', 'deadTime', 'afterpulsingProbability', 'crosstalkProbability', consider their combined impact on QBER and key rate.
